...
for anes_num, anes_time, surgery_time in zip(anes['마취기록작성번호'], anes['(마취기록)마취시작'], anes['(마취기록)수술시작']):
    ...
    if count % 1000 == 0:
        ...
        test.to_excel('/srv/project_data/EMR/jy/Post-induction/Outcomes_preprocessing/outcomes_55.xlsx', index=False, sheet_name='HPI')
    # print(len(test))
test = pd.DataFrame({'anes_num': anesnum_info, 'Hypotension': hpi_info, 'Hypotension_Area': area_info,'Hypotension_Time(minute)': hpicount_info, 'MBP_Max': mean_max, 'MBP_Mean': mean_mean, 'MBP_Min': mean_min})
test.to_excel('/srv/project_data/EMR/jy/Post-induction/Outcomes_preprocessing/20230925_outcomes_55.xlsx', index=False, sheet_name='HPI')
test.to_csv('/srv/project_data/EMR/jy/Post-induction/Outcomes_preprocessing/outcomes_55_csv_ver.csv', index=False)
